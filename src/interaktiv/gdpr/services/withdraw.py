...
class WithdrawDeletion(Service):
    ...
    def reply(self):
        ...
        # Get the log entry
        log_entry = DeletionLogHelper.get_entry_by_uid(self.uid)
        if not log_entry:
            self.request.response.setStatus(404)
            return {
                'error': {
                    'type': 'NotFound',
                    'message': f'No deletion log entry found for UID: {self.uid}'
                }
            }
        ...
        if log_entry['status'] != 'pending':
            ...
            return {
                'error': {
                    'type': 'BadRequest',
                    'message': f'Cannot withdraw: status is "{log_entry["status"]}", expected "pending"'
                }
            }
        ...
        if original_id in target_container.objectIds():
            ...
            return {
                'error': {
                    'type': 'Conflict',
                    'message': f'Name conflict: An object with id "{original_id}" already exists at /{original_parent_path}'
                }
            }
...
class GDPRSettingsSet(Service):
    ...
    def reply(self):
        ...
        if not new_value:
            ...
            if pending_entries:
                ...
                return {
                    'error': {
                        'type': 'Conflict',
                        'message': 'Es sind noch ausstehende Loeschungen aktiv. Loesen Sie diese auf um das Feature zu deaktivieren.',
        ...
        logger.info(f"GDPR marked deletion feature {'enabled' if new_value else 'disabled'}")

        return {
            'status': 'success',
            'marked_deletion_enabled': new_value
        }
